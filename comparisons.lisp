...
(defun each?/=any (x sequence)
  (if sequence
      (each?/=any-internal (?xs-in x) sequence)
    T))

(defun ?comparison-arguments (x list) 
  (let ((xs (remove nil (flatt (append (list x) list)))))
    (if xs xs '(0)))) ; delete
(defun ?< (x &rest xs) (apply #'<v (?comparison-arguments x xs)))
(defun ?> (x &rest xs) (apply #'>v (?comparison-arguments x xs)))
(defun ?<= (x &rest xs) (apply #'<=v (?comparison-arguments x xs)))
(defun ?>= (x &rest xs) (apply #'>=v (?comparison-arguments x xs)))
(defun ?= (x &rest xs) (apply #'=v (?comparison-arguments x xs)))
